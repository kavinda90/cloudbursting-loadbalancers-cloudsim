...
public class HybridCloudTest {
    ...
    private static final int[] HOSTS = {10, 40};
...
//        cloudletFinishedList.sort(sortByVmId.thenComparing(sortByStartTime));
        ...
        new CloudletsTableBuilder(cloudletFinishedList).setTitle(broker0.toString()).build();
...
//        new CloudletsTableBuilder(cloudletFinishedList).build();
    ...
    private Datacenter createDatacenter(final int index) {
        ...
        final var hostList = new ArrayList<Host>(HOSTS[index]);
        for (int i = 0; i < HOSTS[index]; i++) {
