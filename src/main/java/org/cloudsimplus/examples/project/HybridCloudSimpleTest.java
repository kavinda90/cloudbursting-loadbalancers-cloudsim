 ...
 * is set to each {@link #createListOfVms(int) initially created VM},
 ...
 */
public class HybridCloudSimpleTest {
    /**
...
//    private List<Datacenter> datacenterList;
    ...
    public static void main(String[] args) {
        new org.cloudsimplus.examples.project.HybridCloudSimpleTest();
    }

    /**
     * Default constructor that builds the simulation scenario and starts the simulation.
     */
    private HybridCloudSimpleTest() {
...
//        datacenterList = createDatacenters();
        ...
        vmList.addAll(createListOfVms(VMS));
    ...
    private DatacenterBrokerSimple createBroker() {
        ...
        broker.setLastSelectedDc(dc0);
        broker.getVmCreation().setRetryDelay(7.0);
        return broker;
    }
    ...
    private Datacenter createDatacenter() {
        final var newHostList = new ArrayList<Host>(HOSTS);
        for (int i = 0; i < HOSTS; i++) {
            newHostList.add(createHost());
        }

        return new DatacenterSimple(simulation, newHostList).setSchedulingInterval(SCHEDULING_INTERVAL).setHostSearchRetryDelay(7);
    }
     ...
     */
    private List<Vm> createListOfVms(final int vmsNumber) {
